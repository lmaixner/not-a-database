...
def test_assign_id():
	...
	assert len(Table.read(file3)) == len(new_file3)
	assert len(Table.read(file2)) == len(new_file2)

	# check DataNums are as expected
	for row in new_file2['DataNum']:
		assert new_file2['DataNum'][row] == file1['DataNum'][0]
		# if true: matching points are being reassigned to a matching datanum

	for row in new_file3['DataNum']:
		assert new_file3['DataNum'][row] != file1['DataNum'][0]
