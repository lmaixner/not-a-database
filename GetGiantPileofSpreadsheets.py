...
def getFluxes(object1,data1,iR ,oR,G,rN):
	"""
	Preconditions: Expects a sep extracted list of objects and the image data the
	objectlist is from. Can also take the inner radius, outer radius, gain, and
	read noise.
...
def writeTables(ic1,target_dir = 'output',I=innerRad,O=outerRad,G=Tgain,N=readNoise):
	"""
	Preconditions: Requires an image collection and a directory location to put the
	created files. Can also take the inner radius, outer radius, gain, and read noise.
	Postconditions: Writes csv tables containing the information from each image with
	added columns for RA, Dec, flux , fluxerr, InnerRad, OuterRad, Gain, ReadNoise, 
	and Filter.
	"""
	try:
	    os.mkdir(target_dir)
	...
	for hdu, fname in ic1.hdus(imagetyp='light', object='m71', return_fname=True):
	    ...
	    Dmask = np.isnan(data)
	    newdata = np.ma.array(data,mask=Dmask)
	    #data.shape
	    ...
	    object_table = Table(objects) #create a table for the object to fill
	    ra_decs = wcs.all_pix2world(objects['x'], objects['y'], 0) ## retrieve the list of RA/Decs

	    ra_column = Column(data=ra_decs[0], unit='degree', name='RA') ## separate RA column
	    object_table.add_column(ra_column) ##add RA column to table
	    dec_column = Column(data=ra_decs[1], unit='degree', name='Dec') ## separate Dec column
	    ...
	    #print(object_table['RA','Dec','flux','fluxerr','InnerRad','OuterRad','Gain','ReadNoise','Filter'])
